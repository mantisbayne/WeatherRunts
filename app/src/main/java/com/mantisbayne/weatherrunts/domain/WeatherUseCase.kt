...
import com.mantisbayne.weatherrunts.reducer.WeatherDomainState
import com.mantisbayne.weatherrunts.reducer.WeatherDomainStateReducer
    ...
    fun getWeatherData(latitude: Double, longitude: Double): Flow<WeatherDomainState> =
        flow {
            try {
                ...
                emit(
                    WeatherDomainStateReducer.reduceSuccess(response)
                )
            } catch (e: Exception) {
                emit(
                    WeatherDomainStateReducer.reduceError(e.message)
                )
            }
        }
